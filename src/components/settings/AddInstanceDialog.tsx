...
export const AddInstanceDialog = ({ open, onOpenChange }: AddInstanceDialogProps) => {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      if (values.provider_type === 'uazapi' && !showProMode) {
        ...
        if (error) {
          console.error('❌ [AddInstanceDialog] Function error:', error);
          throw error;
        }
        
        if (data?.error) {
          console.error('❌ [AddInstanceDialog] API error details:', JSON.stringify(data.error));
          throw new Error(typeof data.error === 'string' ? data.error : (data.error.message || "Erro na API UazAPI"));
        }
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <Select onValueChange={field.onChange} defaultValue={field.value}>
                            ...
                            <SelectContent>
                              <SelectItem value="uazapi">UazAPI (Recomendado)</SelectItem>
